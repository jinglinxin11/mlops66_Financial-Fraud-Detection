...
def log_inference(inputs_dict, prediction, probability, status):
    ...
    with open(LOG_FILE, mode='a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            header = ["timestamp"] + list(inputs_dict.keys()) + ["prediction", "probability", "status"]
            writer.writerow(header)
        
        row = [datetime.now().isoformat()] + list(inputs_dict.values()) + [prediction, probability, status]
        writer.writerow(row)
...
def ensure_loaded():
    ...
    try:
        ...
        if preprocessor.encoder.label_encoders:
            ...
            DATA = {"X_test": np.random.rand(100, 10), "x_valid": np.random.rand(100, 10), "transaction_ids": np.arange(3000000, 3000100)}
...
def load_data_explorer():
    ...
    if 'isFraud' in df.columns:
        ...
        fig_dist = px.pie(values=counts.values, names=['Normal', 'Fraud'], title="Target Distribution", color_discrete_map={'Normal': 'green', 'Fraud': 'red'})
    else:
        fig_dist = px.pie(values=[90, 10], names=['Normal', 'Fraud'], title="Target Distribution (Mock)")
...
def train_model(epochs, lr, batch_size, n_d, n_a, gamma, device):
    ...
    if IS_TRAINING: yield "⚠️ Training in progress!", None, None, None; return
    ...
    def _run_train():
        ...
        try:
            ...
            if not is_mock:
                ...
                data = {"X_train": X_train, "y_train": y_train, "X_valid": X_train[:100], "y_valid": y_train[:100], "cat_idxs": [], "cat_dims": [], "feature_columns": [f"f{i}" for i in range(10)]}
            ...
            if 'valid_auc' in history:
                fig_auc = go.Figure()
                fig_auc.add_trace(go.Scatter(y=history['valid_auc'], mode='lines+markers', name='AUC'))
    ...
    while IS_TRAINING:
        yield "\n".join(TRAINING_LOGS), None, None, None
        time.sleep(0.5)
    yield "\n".join(TRAINING_LOGS), result_holder["loss_fig"], result_holder["auc_fig"], result_holder["metrics"]

def load_evaluation():
    if 'feature_importances' not in TRAIN_RESULT_CACHE: return None, None, "⚠️ Run training first."
    imps = TRAIN_RESULT_CACHE['feature_importances']
    names = TRAIN_RESULT_CACHE['feature_names']
    indices = np.argsort(imps)[::-1][:20]
    fig_imp = px.bar(x=imps[indices], y=np.array(names)[indices], orientation='h', title="Top 20 Features")
    fig_imp.update_layout(yaxis={'categoryorder':'total ascending'})
    cm = confusion_matrix(TRAIN_RESULT_CACHE['y_true'], TRAIN_RESULT_CACHE['y_pred'])
    fig_cm = px.imshow(cm, text_auto=True, title="Confusion Matrix", x=['Normal', 'Fraud'], y=['Normal', 'Fraud'])
    return fig_imp, fig_cm, classification_report(TRAIN_RESULT_CACHE['y_true'], TRAIN_RESULT_CACHE['y_pred'])
...
def predict_smart(amt, card1, dist1, addr1, p_email):
    ...
    try:
        if len(input_vector) > 0: input_vector[0] = float(amt) or 0
        if len(input_vector) > 1: input_vector[1] = float(card1) or 0
        if len(input_vector) > 2: input_vector[2] = float(dist1) or 0
        if len(input_vector) > 3: input_vector[3] = float(addr1) or 0
    except: pass
    
    X = input_vector.reshape(1, -1)
    try: proba = MODEL.predict_proba(X)[0, 1]
    except: proba = 0.5 
...
def auto_finetune(amt, card1, dist1, addr1, p_email, correct_label):
    ...
    try:
        ...
        log_inference(
            ...
            f"Corrected_to_{correct_label}", 1.0, "Finetuned"
        )
    ...
    with gr.Blocks(title="Fraud Detection Ultimate") as demo:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("🖥️ System Monitor"):
                ...
                refresh_btn.click(refresh_system_metrics, outputs=[cpu_box, ram_box, disk_box, cpu_gauge, ram_gauge])
            ...
            with gr.TabItem("🚀 Advanced Training"):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        logs = gr.Textbox(label="Real-time Logs", lines=12, max_lines=12, interactive=False)
                        with gr.Row():
                            p_loss = gr.Plot(label="Loss")
                            p_auc = gr.Plot(label="AUC")
                
                btn.click(train_model, [ep, lr, bs, nd, na, gam, dev], [logs, p_loss, p_auc, metric_txt])
            ...
            with gr.TabItem("🔮 Ops & Logging"):
                ...
                smart_btn.click(predict_smart, [in_amt, in_card, in_dist, in_addr, in_email], [smart_res, smart_prob])
                
                gr.Markdown("---")
                gr.Markdown("### 🔄 M27: Drift Robustness (Auto-Finetune)")
                with gr.Row():
                    correct_lbl = gr.Radio(["Normal (0)", "Fraud (1)"], label="Correct Label", value="Normal (0)")
                    finetune_btn = gr.Button("🛠️ Correct & Learn", variant="stop")
                
                finetune_res = gr.Textbox(label="Finetune Status")
                finetune_btn.click(auto_finetune, [in_amt, in_card, in_dist, in_addr, in_email, correct_lbl], finetune_res)
                
                gr.Markdown("---")
                gr.Markdown("*Note: All predictions are automatically logged to `data/inference_history.csv` (M27 Data Collection).*")
...
if __name__ == "__main__":
    ...
    """
    app = build_app()
    app.launch(server_name="127.0.0.1", server_port=7860, show_error=True, css=css, theme=gr.themes.Soft())
