...
def test_init():
    ...
    assert loader.verbose is False, "DataLoader verbose flag should be False when initialized with verbose=False"
    
    loader_verbose = DataLoader(verbose=True)
    assert loader_verbose.verbose is True, "DataLoader verbose flag should be True by default or when initialized with True"
...
def test_load_and_merge(loader, sample_data):
    ...
    assert 'device_info' in merged_df.columns, "Merged data should contain identity columns (device_info)"
    
    # Check left join behavior (TransactionID 3 has no identity info)
    row_3 = merged_df[merged_df['TransactionID'] == 3]
    assert pd.isna(row_3['device_info'].iloc[0]), "Expected NaN for missing identity match in left join"
...
def test_load_and_merge_no_identity(loader, sample_data):
    ...
    assert 'device_info' not in df.columns, "Result should not contain identity columns when identity_path is None"
