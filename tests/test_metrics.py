...
def test_evaluate_model(trained_model, dummy_data):
    ...
    assert metrics['proba'].shape[0] == X.shape[0], "Probability predictions should match number of samples"
...
def test_feature_importance(trained_model, dummy_data):
    ...
    assert len(importance_df) == X.shape[1], "Feature importance should have an entry for each feature"
    assert 'importance' in importance_df.columns, "Feature importance DataFrame should have 'Importance' column"
    # Check if importance is numeric
    assert np.issubdtype(importance_df['importance'].dtype, np.number), "Importance values should be numeric"
...
def test_evaluate_model_integration(trained_model, dummy_data):
    ...
    assert len(metrics['feature_importance']) == X.shape[1], "Feature importance should have an entry for each feature"
